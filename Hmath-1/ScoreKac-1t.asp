...
<HR>
 ...
 <A HREF="Checkac-1t.asp?No=<%=No%>&Name=<%=Name%>&Lesson=<%=Lesson%>">核對答案</A>
...
%>
   ...
   '' End If
    %>
   <FORM >
     <INPUT Type=Button Value="(返回上上一頁)重新輸入" OnClick="history.back();">
        <!--<A HREF="Enterkac-1.asp">重新進入測試</A>
     <A HREF="login-1t.asp">重新進入測試</A>--> 
     <A HREF="/Hmath/Member-1r/Hmath/.asp">重新進入登錄</A>
   </FORM>
   </CENTER>
