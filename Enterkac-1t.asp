...
If Request("Send") <> Empty Then
  ...
  ''cmd.CommandText = SQLD
    SQLS1 ="Select * into "&Lesson&Trim(Cstr(No))& " FROM " &Lesson& " Order by 題號" 
  ...
  'SQL1 ="Insert into ASP Select * FROM "&Lesson&"K"&"Where 標記=+1 Order by 題號"
   SQL1 ="Insert into "&Lesson&Trim(Cstr(No))& " Select * From "&Lesson&"K"&" Where 標記=100 Order by 題號 "
   ...
   For j=1 to 2
      ...
      While Not rs.EOF
        if rs("流水號")=iRN AND rs("標記")=-1 then
              rs("標記")=100
              rs("題目碼")=rs("流水號")
                crsTN ="<A HREF =/"&Trim(Cstr(rs("流水號")))&".htm"&">詳答按此</A>"
                rs("詳答") = crsTN
                 ' crsTN ="<A HREF =Hmath-1t/"&Trim(Cstr(rs("流水號")))&".htm"&">詳答按此</A>"
                 ' rs("詳答") = crsTN
                n=n+1
               rs("題號")=iRN
               'rs("題號")=n
